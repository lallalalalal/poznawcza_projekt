...
expName = 'navon_task'  # from the Builder filename that created this script
expInfo = {
    'płeć': '',
    'wiek': '',
    'wariant ': '',
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Desktop\\psycho.py labolatoria\\navon_task_kotki_lastrun.py',
...
trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('cats_pic.xlsx'),
...
for thisTrial_2 in trials_2:
    ...
    cat_pic.setImage(cat_pics)
    # keep track of which components have finished
    primingComponents = [cat_pic]
    ...
    while continueRoutine and routineTimer.getTime() < 1.5:
        ...
        # *cat_pic* updates
        if cat_pic.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            cat_pic.frameNStart = frameN  # exact frame index
            cat_pic.tStart = t  # local t and not account for scr refresh
            cat_pic.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(cat_pic, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'cat_pic.started')
            cat_pic.setAutoDraw(True)
        if cat_pic.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cat_pic.tStartRefresh + 1.5-frameTolerance:
                # keep track of stop time/frame for later
                cat_pic.tStop = t  # not accounting for scr refresh
                cat_pic.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'cat_pic.stopped')
                cat_pic.setAutoDraw(False)
...
session = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('global_first.xlsx'),
...
for thisSession in session:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    introduction.setImage(exp_intro)
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    introComponents = [introduction, key_resp_2]
    ...
    session.addData('key_resp_2.keys',key_resp_2.keys)
    if key_resp_2.keys != None:  # we had a response
        session.addData('key_resp_2.rt', key_resp_2.rt)
    ...
    trials = data.TrialHandler(nReps=2.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(gl_level),
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['s','h'], waitRelease=False)
